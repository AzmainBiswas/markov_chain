...
\begin{definition}[Markov Chain]
    A discrete time stochastic process $\{X_n,n=1,2,3,\ldots\}$ is defined to be \textit{Discrete Time Markov Chain} or simply \textit{Markov Chain} if it takes value in the state space $ \mathbf{S} $, and for every
...
Unless otherwise mentioned we take the state space $ \mathbf{S} $ to be $\{ 1, 2, 3, \ldots\ \} $. 
